...
XmlClass::Result XmlDagReader::Fill(const XERCES_CPP_NAMESPACE::DOMNode & anElement, XmlDag & aDag)
{
    ...
    for (XMLSize_t i = 0; i < list->getLength(); i++) {
     ...
     else if (wcscmp(node->getNodeName(), L"bonuscell") == 0)
     {
    ...
    {
    const XERCES_CPP_NAMESPACE::DOMNode* bonuspuntennode = attributes->getNamedItem(L"bonuspunten");
    if (bonuspuntennode != nullptr)
      aDag.Setbonuspunten(Str::ToString(bonuspuntennode->getNodeValue()));
    const XERCES_CPP_NAMESPACE::DOMNode* datumnode = attributes->getNamedItem(L"datum");
    if (datumnode != nullptr)
      aDag.Setdatum(Str::ToString(datumnode->getNodeValue()));
    const XERCES_CPP_NAMESPACE::DOMNode* gewichtnode = attributes->getNamedItem(L"gewicht");
    if (gewichtnode != nullptr)
      aDag.Setgewicht(Str::ToString(gewichtnode->getNodeValue()));
    }
