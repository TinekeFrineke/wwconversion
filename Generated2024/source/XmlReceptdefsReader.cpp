...
std::unique_ptr<XmlReceptdefs> XmlReceptdefsReader::Read(const std::string & aFilename)
{
    ...
    try
    {
        ...
        for (XMLSize_t i = 0; i < list->getLength(); i++)
        {
            ...
            {
                if (wcscmp(node->getNodeName(), L"receptdefs") == 0)
...
XmlClass::Result XmlReceptdefsReader::Fill(const XERCES_CPP_NAMESPACE::DOMNode & anElement, XmlReceptdefs & aReceptdefs)
{
    ...
    for (XMLSize_t i = 0; i < list->getLength(); i++)
    {
        ...
        {
            if (wcscmp(node->getNodeName(), L"receptdef") == 0)
     {
